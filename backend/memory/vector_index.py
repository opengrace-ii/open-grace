...
class SentenceTransformerProvider(EmbeddingProvider):
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2"):
        ...
        self._model = None
        self.dimension = 384  # default for all-MiniLM-L6-v2, will be updated on load
    
    def _load_model(self):
        """Actually load the model (synchronous, to be run in thread)."""
    ...
    async def embed(self, texts: List[str]) -> List[List[float]]:
        ...
        # Ensure model is loaded (in thread pool)
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            for text in texts:
                try:
                    ...
                    # Return empty vector of correct dimension if possible
...
class VectorStore:
    ...
    async def add_documents(self, documents: List[tuple]) -> List[Document]:
        """
    ...
    async def search(self, query: str, top_k: int = 5, 
        ...
        if self.backend == "faiss":
            ...
            return await self._search_chroma(query, top_k, filter_metadata)
        return []
    
    async def _search_faiss(self, query: str, top_k: int) -> List[SearchResult]:
        """Search using FAISS."""
    ...
    async def _search_chroma(self, query: str, top_k: int,
                       filter_metadata: Optional[Dict[str, Any]]) -> List[SearchResult]:
        """Search using Chroma."""
        # Query Chroma (note: chromadb doesn't have native async, but this avoids blocking if pushed to thread)
        # For simplicity, keeping as is but marked async for interface consistency
